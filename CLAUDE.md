...
- `POST /auth/forgot-password` - Send password reset email (`@Public()`, returns `{ message }`; never reveals whether the email exists)
...
37. **Deploy script invariants** (`Deploy-to-VPS.ps1`):
    ...
    - **Frontend build exit code**: `Build-Frontend` captures `$LASTEXITCODE` from `pnpm --filter frontend build` and aborts the deploy on non-zero. Don't pipe the build to `Out-Null` and don't use `Test-Path dist/` as a success signal — Vite leaves the previous successful `dist/` in place when `tsc -b` fails first, so a TypeScript regression silently re-ships the last good bundle (this happened: a stale Apr 29 bundle re-deployed on every run for a full day before someone noticed the UI hadn't changed). If the build fails, fix the TS errors before retrying — never bypass the gate.
...
39. **Forgot/reset password feature**: end-to-end flow lives at `/auth/forgot-password` (request reset link) and `/auth/reset-password?token=...` (set new password). `authApi.forgotPassword({email})` and `authApi.resetPassword({token, password})` call `POST /auth/forgot-password` and `POST /auth/reset-password` respectively (both `@Public()`). Service-layer `AuthService.forgotPassword` generates a token, persists `passwordResetToken` + `passwordResetExpires` on the user (24h TTL), and emails the link via `MailService.sendPasswordReset`. The forgot endpoint never reveals whether the email exists (returns the same `{ message: "If an account exists..." }` for both branches) — anti-enumeration. `MailService.sendPasswordReset` requires the `SMTP_FROM` domain to be verified at the emailit provider; an unverified `From:` returns `530 From/Sender domain is not verified` and the email never sends. Login page links to `/auth/forgot-password` via the standard "Forgot password?" link.
