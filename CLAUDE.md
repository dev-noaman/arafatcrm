...
39. **In-flight forgot/reset password WIP**: `frontend/src/pages/auth/ForgotPasswordPage.tsx` and `ResetPasswordPage.tsx` exist (untracked) and call `authApi.forgotPassword()` / `authApi.resetPassword()`, but those methods aren't yet exported from `frontend/src/api/auth.ts` — `pnpm --filter frontend build` fails on `Property 'forgotPassword' does not exist...`. The matching backend endpoints (`POST /auth/forgot-password`, `POST /auth/reset-password`) and the `password_reset_token` / `password_reset_expires` columns on `users` are already present from migration `1778200000000`. To finish the feature: add the two methods to `authApi` calling those endpoints, wire the routes into `App.tsx`, and verify the `/auth/forgot-password` flow end-to-end. Until then, every deploy will fail at the Build-Frontend gate (note 37) and refuse to ship a stale bundle — by design.
